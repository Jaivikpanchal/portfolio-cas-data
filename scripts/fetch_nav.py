...
#     """
#     Proper XIRR using Newton-Raphson method.
#     cashflows: list of (date, amount)
#     Negative = money invested (outflow), Positive = current value (inflow)
#     """
#     if len(cashflows) < 2:
#         return None
...
#     return round(rate * 100, 2) if -99 < rate * 100 < 1000 else None
...
#                 cashflows.append((d, -t["invested"]))  # outflow (negative)
#             except ValueError:
#                 pass
#         if cashflows:
#             cashflows.append((datetime.now(), total_value))  # inflow (positive)
#             cashflows.sort(key=lambda x: x[0])
#             xirr = xirr_calc(cashflows)
#     except Exception as e:
#         print(f"  XIRR calc error: {e}")
